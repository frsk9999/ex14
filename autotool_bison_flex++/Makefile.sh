...
sed -e "s/AC_CONFIG_SRCDIR\(.*\)/AC_CONFIG_SRCDIR([test.cc])/" | \
...
autoheader
aclocal
echo "BUILT_SOURCES=test_yacc.hh"                                              > Makefile.am
echo "AM_YFLAGS = -d"                                                          >>Makefile.am
echo "bin_PROGRAMS=test"                                                       >>Makefile.am
echo "test_SOURCES=test_yacc.yy test_lex.ll test.cc test_driver.cc config.h"   >>Makefile.am
automake -a -c
autoconf
./configure
# あとはmakeするだけ
